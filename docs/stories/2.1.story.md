...
- [ ] **AC1:** Beginner AI selects valid moves from the current board state using `OwareRules.GetValidMoves()`
- [ ] **AC2:** AI makes random moves 70% of the time from available valid moves
- [ ] **AC3:** AI applies strategic heuristic 30% of the time:
  - Prioritize moves that capture seeds (2-3 seeds in opponent pit after sowing)
  - Prioritize moves that prevent opponent from capturing on their next turn
- [ ] **AC4:** AI move selection completes within 100ms on target devices (no perceived lag)
- [ ] **AC5:** AI integrates with existing `GameManager.TriggerAIMove()` method with 1.0-1.5s delay for human-like feel
- [ ] **AC6:** AI respects all Oware rules (Grand Slam, forced feeding) via `OwareRules.ExecuteMove()`
- [ ] **AC7:** Game Manager correctly identifies when Beginner AI is active and invokes it during AI turns

### Technical Requirements

- [ ] **TR1:** Implement `BeginnerAI` class following the `IOwareAI` interface pattern defined in architecture
- [ ] **TR2:** Use `OwareBoard.Clone()` for move simulation (heuristic evaluation) without mutating game state
- [ ] **TR3:** Follow C# coding standards from architecture (PascalCase classes, camelCase private fields, XML documentation)
- [ ] **TR4:** No MonoBehaviour usage - pure C# class for testability
- [ ] **TR5:** Code maintains 60 FPS performance (AI logic off main thread or <16ms execution)

### Game Design Requirements

- [ ] **GD1:** AI provides winnable challenge for players learning Oware rules (target: 60-70% player win rate)
- [ ] **GD2:** AI move delay (1.0-1.5s) creates natural pacing, not instant responses
- [ ] **GD3:** Strategic heuristic creates occasional "smart" moves that teach players defensive play
...
- [ ] **Task 1:** Create `AI` folder in `Assets/_Project/Scripts/Gameplay/`
- [ ] **Task 2:** Implement `IOwareAI.cs` interface with `SelectMove(OwareBoard)` method signature
- [ ] **Task 3:** Implement `BeginnerAI.cs` class:
...
- [ ] **Task 5:** Write unit tests for `BeginnerAI`:
...
- [ ] All acceptance criteria met (AC1-AC7, TR1-TR5, GD1-GD3)
- [ ] Code reviewed and approved (peer review or self-review if solo)
- [ ] Unit tests written and passing (BeginnerAITests.cs with 4+ test methods)
- [ ] PlayMode manual testing completed (4 test cases above)
- [ ] Performance targets met (60 FPS, <100ms AI execution)
- [ ] No C# compiler errors or warnings
- [ ] XML documentation added to all public methods
- [ ] Code follows architecture coding standards (naming, structure, patterns)
- [ ] GameManager Inspector shows AI Difficulty dropdown with "Beginner" option
- [ ] AI move selection logged to Console with pit index for debugging
